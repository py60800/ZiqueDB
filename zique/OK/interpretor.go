...
func (p *Player) AddSwing(s SeqEvent) SeqEvent {
	if len(p.SwingFactor) > 0 {
		...
		//		fmt.Println("Sw:", s.Tick, nTick, s.Tick-p.MeasStart, nTick-p.MeasStart)
...
func (p *Player) PurgeAuxSequence() {
	...
	chordPurged := false
	drumPurged := false
	fmt.Println("Purge :", len(p.AuxSequence), "Clock:", p.Clock)
	for _, auxEvent := range p.AuxSequence {
		switch v := auxEvent.Event.(type) {
		case *PChordOff:
			for _, n := range v.Notes {
				fmt.Println("Purge:", p.Clock, n)
	...
	fmt.Println("Purge done", len(p.AuxSequence))
}
...
func (p *Player) PartInit(pm *MPart) {
	...
	fmt.Printf("PartInit XmlDivisions: %v, Beats: %v, BeatsType: %v Mlength:%v\n", p.XmlDivisions, p.Beats, p.BeatType, (p.Beats*MasterDivisions*4)/p.BeatType)
}
...
func (p *Player) processCmd(C CPlayCtrl) bool {
	...
	case CVELOCITY:
		...
		fmt.Println("Velocity p:", C.Param, p.VelocityPattern)
		p.ComputeVelocity()
	default:
...
loop:
	for {
		...
		if iter.IsStartMeasure() || Replay {
			fmt.Printf("Measure: %3d (%v) : Prev", iter.CurrentMeasure().Id, p.Clock)
			{
				for _, l := range p.MNoteLength {
					fmt.Printf(" %1.2f/%1.2f", float64(l.tickOn-p.MeasStart)/float64(p.XmlDivisions),
						float64(l.tickOf-l.tickOn)/float64(p.XmlDivisions))
				}
...
func (p *Player) AddDrum(Clock uint32, actualMeasureLength uint32) {
	...
	fmt.Printf("Ab Drum StartM %v, MLen: %v len:%v refL:%v, Seq:%v\n",
		sClock, actualMeasureLength, len(p.AuxSequence), len(p.DrumPattern), p.AuxSequence)
}
